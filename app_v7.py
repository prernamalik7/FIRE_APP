...
if run:
    ...
    # =========================
    # Ultra User-Friendly Wealth Projection
    # =========================
    ...
    # Plot lines
    ax.plot(df_yearly["Age"], df_yearly["Nominal Portfolio (CAD)"],
            linewidth=4, color=nominal_color)

    ax.plot(df_yearly["Age"], df_yearly["Real Portfolio (CAD)"],
            linewidth=4, linestyle="--", color=real_color)

    # FIRE target line
    ax.axhline(fire_number, linestyle=":", linewidth=3, color=fire_color)

    # Retirement shading
    ax.axvspan(retirement_age, life_expectancy,
    ...
    ax.scatter(age, current_value, s=150, color="black", zorder=5)

    ax.annotate("You Are Here",
                xy=(age, current_value),
                xytext=(age+1, current_value*1.1),
    ...
    if fire_age:
        ...
        ax.scatter(fire_age, fire_value,
                s=200, color="gold", zorder=5)

        ax.annotate("Work Becomes Optional Here",
                    xy=(fire_age, fire_value),
                    xytext=(fire_age-5, fire_value*1.15),
                    arrowprops=dict(arrowstyle="->", lw=2),
                    fontsize=13,
                    fontweight="bold")

    # -------------------------
    # Inline Line Labels (instead of legend)
    # -------------------------
    ax.text(df_yearly["Age"].iloc[-1] + 0.5,
            df_yearly["Nominal Portfolio (CAD)"].iloc[-1],
            " Portfolio Value (Actual Dollars)",
            color=nominal_color,
            fontsize=12,
            fontweight="bold",
    ...
    ax.text(df_yearly["Age"].iloc[-1] + 0.5,
            df_yearly["Real Portfolio (CAD)"].iloc[-1],
    ...
    # -------------------------
    # Funny Market Dip Callouts
    # -------------------------
    for i in range(1, len(df_yearly)):
        if df_yearly["Net Gain (Nominal)"].iloc[i] < 0:
            dip_age = df_yearly["Age"].iloc[i]
            dip_val = df_yearly["Nominal Portfolio (CAD)"].iloc[i]

            ax.scatter(dip_age, dip_val,
                    s=80, color="purple", zorder=5)

            ax.annotate("Market Had a Bad Day",
                        xy=(dip_age, dip_val),
                        xytext=(dip_age, dip_val*0.85),
                        fontsize=9,
                        color="purple",
                        arrowprops=dict(arrowstyle="->", lw=1))
            break  # only mark first dip to avoid clutter

    # -------------------------
    # Formatting
    # -------------------------
    ax.set_title("Your Road to Financial Independence",
                fontsize=18,
                fontweight="bold")

    ax.set_xlabel("Your Age", fontsize=13)
    ax.set_ylabel("Portfolio Value (Dollars)", fontsize=13)

    ax.yaxis.set_major_formatter(
        FuncFormatter(lambda x, _: f"${x:,.0f}")
    )

    ax.set_xlim(age - 1, life_expectancy + 3)
    ax.grid(alpha=0.25)
    ...
    st.caption(
        "Solid line = actual projected portfolio. "
        "Dashed line = inflation-adjusted value (today's buying power). "
        "Shaded region = retirement years. "
        "Markers highlight important moments along your journey."
    )
